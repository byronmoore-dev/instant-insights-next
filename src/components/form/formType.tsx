...
export const TextArea = ({ updateForm, id, label }: { updateForm: (arg0: any) => void; id: string; label: string }) => {
  return (
    <div className="relative mb-4 mt-4 w-full overflow-hidden rounded-2xl border-[1.5px] border-l-border bg-l-foreground py-2 pr-2 dark:border-d-border dark:bg-d-foreground">
      <textarea
        ...
        onChange={updateForm}
      />
      <label className="pointer-events-none absolute left-6 top-6 w-[90%] text-l-text-second duration-200 peer-valid:top-2 peer-valid:pb-1 peer-valid:text-xs peer-focus:top-2 peer-focus:pb-1 peer-focus:text-xs dark:text-d-text-second">
        {label}
      </label>
    </div>
  );
